...
const MusicCard = ({ musicData, isPlaying, isGrid = false, onPlay }: any) => {

    console.log(musicData, "+66")
  ...
  return (
    ...
    >
      <div className={`rounded-xl bg-white/[0.1] opacity-[0.4] overflow-hidden ${isGrid ? "w-full h-[180px]" : "w-[100px] h-[100px]"}`}>
        <Image src={musicData?.image ?? DefaultImage} alt={musicData?.name} width={500} height={500} className="w-full h-full object-cover" />
      </div>
